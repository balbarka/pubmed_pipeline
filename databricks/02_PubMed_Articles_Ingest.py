...
# MAGIC %md
# MAGIC
# MAGIC # Process downloaded articles
# MAGIC
...
# pyspark dataframe columns to write in PubMed curated articles delta table
writeStream_columns = [F.col("AccessionID"),
                      F.col("ETag"),
                      F.col("LastUpdated"),
                      F.col("PMID"),
...
spark.readStream.format("delta") \
  ...
  .table(pubmed.raw_metadata.name) \
  .filter("Status == 'DOWNLOADED'") \
  .filter("_change_type == 'update_postimage'") \
  .filter("Retracted == 'no'") \
  .repartition(64) \
  .withColumn("parsed", curate_xml_dict("volume_path")) \
  .select(writeStream_columns) \
  .writeStream \
  .trigger(availableNow=True) \
  .option("checkpointLocation", pubmed.curated_articles.cp.path) \
  .toTable(pubmed.curated_articles.name) \
  .awaitTermination()
...
# MAGIC %md
# MAGIC
# MAGIC # Inspect curated_articles (OPTIONAL)

# COMMAND ----------

# MAGIC %sql
# MAGIC SELECT * FROM `pubmed-pipeline`.curated.articles_xml
